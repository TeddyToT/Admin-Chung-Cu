...
const RoomTable = ({ filterStatus, setFilterStatus }) => {
  const { rooms, fetchRooms }: any = useContext(Contexts);
  // fetchrooms();
  ...
  useEffect(() => {
    ...
    if (filterStatus !== "all") {
      filtered = rooms.filter(
        (room) => room.isAvailable.toString() === filterStatus
      );
    }
  ...
  const handleDeleteProduct = (productId: string) => {
    axios
      .delete(`http://localhost:8081/v1/api/user/rooms/${productId}`)
      .then((response) => {
  ...
  const totalPages = Math.ceil(searchrooms.length / itemsPerPage);
  const handleNext = () => {
  ...
  const getPaginatedData = () => {
    ...
    const sortedOrders = [...searchrooms].sort(
  ...
  return (
    <div className="mb-5 rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      <div className=" flex flex-row justify-start">
        <div className=" w-1/2 px-4 py-5 sm:block">
            <div className="relative">
               
                <SearchOutlinedIcon className="absolute left-0 top-1/2 -translate-y-1/2" />
              <input
                type="text"
                value={searchInput}
        ...
        >
          ...
          <div className="col-span-1 flex items-center justify-center">
            <div className="flex items-center space-x-3.5">
              <Link
                href={`/room/overview/edit-room/${product._id}`}
